...
def export_excel():
    ...
    df = pd.DataFrame([{'Codice': i.code, 'Matricola': i.serial, 'Descrizione': i.description, 'Qta': i.quantity} for i in items])
    out = BytesIO()
    df.to_excel(out, index=False)
    out.seek(0)
    return send_file(out, download_name="Inventario_Evolve.xlsx", as_attachment=True)
